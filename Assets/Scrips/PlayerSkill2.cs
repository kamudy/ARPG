...
public class PlayerSkill2 : MonoBehaviour
{
    ...
    public void TryUseSkill2()
    {
        ...
        if (mana != null)
        {
            ...
            {
                if (logHits) Debug.Log("❌ No hay mana para Skill2.");
                return;
            }
        }
    ...
    System.Collections.IEnumerator DelayedSkillEffect()
    {
        ...
        if (logHits) Debug.Log($"✨ Skill2 ejecutada | -{manaCost} mana | dmg={damage}");
    ...
    void RotateToMouse()
    {
        ...
        if (Physics.Raycast(ray, out RaycastHit hit, 200f, groundLayer))
        {
            ...
            if (dir.sqrMagnitude > 0.001f)
            {
                ...
                if (logHits) Debug.Log($"✓ Skill2: Rotado hacia mouse. Forward: {transform.forward}");
            }
        }
        else
        {
            ...
            if (mouseWorldPos != Vector3.zero)
            {
                ...
                {
                    transform.rotation = Quaternion.LookRotation(dir);
                    if (logHits) Debug.Log($"⚠ Skill2: Raycast falló. Usado fallback. Forward: {transform.forward}");
                }
            }
        }
    }
    ...
    void DoDamageAndImpact()
    {
        ...
        foreach (Collider collider in hits)
        {
            ...
            if (enemy != null)
            {
                ...
                if (logHits) Debug.Log($"💥 Skill2 golpeó a {collider.name}: -{damage} HP");
            }
        }
    }
    ...
    void SpawnSkillVFX()
    {
        ...
        if (logHits) Debug.Log($"📍 Skill2 VFX spawneado en {spawnPos}");
    }
